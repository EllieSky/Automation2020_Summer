...
class Homework5(unittest.TestCase):
    ...
    def test_add_employee(self):
        ...
        name = "Mark"
        first_name = browser.find_element_by_id("firstName").send_keys(name)
        ...
        wait.until(expected_conditions.visibility_of_element_located((By.CSS_SELECTOR, "user_name")))
        user_name_field = browser.find_element_by_id("user_name")
        password_field = browser.find_element_by_id("user_password")
        password = "mw123456"
        # 4) Wait for the 'User Name' and 'Password' fields to appear
        wait.until(expected_conditions.visibility_of(user_name_field))
        ...
        user_name_field.send_keys(name + user_id)
        ...
        # actual_message =browser.find_element_by_id("welcome").click()
        wait.until(expected_conditions.visibility_of_element_located((By.LINK_TEXT, "Logout")))

        browser.find_element_by_id("txtUsername").send_keys(user_name_field)
        browser.find_element_by_id("txtPassword").send_keys(password)
        browser.find_element_by_id("btnLogin").click()
        actual_message = browser.find_element_by_id("welcome").text

        self.assertTrue("Welcome Mark", actual_message)
